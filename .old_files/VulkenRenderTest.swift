...
public enum RenderNodeEnum<A, B, C> where A: VulkanRenderNode, B: VulkanThorRenderNode, C: VulkanSkiaRenderNode {
    case normal(A)
    case thor_canvas(B)
    case skia_canvas(C)
}
...
// MARK: - Concrete demo node

final class DemoShaderNode: VulkanRenderNode {
    let image:                VkImage
    let imageView:            VkImageView
    var dirty:                Bool = true
    let computePipeline:      VkPipeline?
    let computeLayout:        VkPipelineLayout?
    let computeDescriptorSet: VkDescriptorSet?
...
// MARK: - Image barrier helper

private func imageBarrier(
    _ cmd:     VkCommandBuffer,
    image:     VkImage,
    srcLayout: VkImageLayout,
    srcAccess: VkAccessFlags,            // UInt32 — pass VkAccessFlags(VK_ACCESS_xxx.rawValue)
    srcStage:  VkPipelineStageFlagBits,  // enum WITH Bits — converted to UInt32 inside
...
/// Updates all dirty canvas nodes (ThorVG draw + barriers + optional compute),
/// then records the composite draw. Call inside your render pass.
/// Returns without touching composite if nothing was dirty.
func renderFrame<Node: VulkanThorRenderNode>(
    slots:         [Node],
    commandBuffer: VkCommandBuffer,
    composite:     CompositePipeline,
    compositeSet:  VkDescriptorSet,
    viewport:      VkViewport,
    scissor:       VkRect2D
) {
    var anyDirty = false

    for slot in slots where slot.dirty {
        
        guard
            slot.canvas.draw() == TVG_RESULT_SUCCESS,
            slot.canvas.sync() == TVG_RESULT_SUCCESS
        else {
            print(slot, "failed rendering")
            continue
        }
        ...
        if let pipeline = slot.computePipeline,
           let layout   = slot.computeLayout,
           let ds       = slot.computeDescriptorSet {

            imageBarrier(
                commandBuffer,
                image:     slot.image,
                srcLayout: VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                srcAccess: VkAccessFlags(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT.rawValue),
            ...
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, layout, 0, 1, &descSet, 0, nil)
            vkCmdDispatch(commandBuffer, (slot.width + 7) / 8, (slot.height + 7) / 8, 1)
            imageBarrier(
                commandBuffer,
                image:     slot.image,
